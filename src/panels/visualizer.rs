use egui::*;
use super::theme;
use crate::spectrum::{SpectrumData, NUM_BANDS, get_band_frequency, frequency_to_note};
...
impl VisualizerState {
    ...
    pub fn update(&mut self, target: &SpectrumData, dt: f32) {
        ...
        for i in 0..NUM_BANDS {
            ...
            self.display_bands_right[i] = lerp(self.display_bands_right[i], target.bands_right[i], speed.min(1.0));
            ...
            if target.peaks_right[i] > self.display_peaks_right[i] {
                ...
                self.display_peaks_right[i] = lerp(self.display_peaks_right[i], target.peaks_right[i], 2.0 * dt);
            }
        }
...
pub fn render_spectrum_visualizer(
    ...
    Frame::default()
        ...
        .show(ui, |ui| {
            ...
            // Header
            ui.horizontal(|ui| {
                ui.label(RichText::new("📊 Spectrum Analyzer")
                    .strong()
                    .size(14.0)
...
fn render_spectrum_with_waterfall(
    ...
    let spectrum_rect = Rect::from_min_max(
        rect.min,
        pos2(rect.max.x, rect.min.y + spectrum_height),
    );
    
    let waterfall_rect = Rect::from_min_max(
        pos2(rect.min.x, rect.min.y + spectrum_height),
        rect.max,
    );
    
    // Draw spectrum on top
    render_spectrum_bars(painter, spectrum_rect, state, enabled, stereo_mode, show_labels);
    ...
    for row in 0..WATERFALL_HISTORY {
        ...
        for band in 0..NUM_BANDS {
            ...
            let pixel_rect = Rect::from_min_max(
                pos2(x, y),
                pos2(x + bar_width - 2.0, y + pixel_height),
            );
    ...
    if stereo_mode {
        ...
        for i in 0..NUM_BANDS {
            ...
            if enabled && peak_value_left > 0.01 {
                painter.line_segment(
                    [pos2(x_left, peak_y_left), pos2(x_left + effective_bar_width, peak_y_left)],
            ...
            if enabled && peak_value_right > 0.01 {
                painter.line_segment(
                    [pos2(x_right, peak_y_right), pos2(x_right + effective_bar_width, peak_y_right)],
                    Stroke::new(2.0, Color32::WHITE),
                );
            }
        }
